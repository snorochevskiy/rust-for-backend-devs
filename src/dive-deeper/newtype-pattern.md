...
Метод `.sort()` требует, чтобы тип сортируемых объектов реализовал трэйт [Ord](https://doc.rust-lang.org/std/cmp/trait.Ord.html). Тип [std::fs::File](https://doc.rust-lang.org/std/fs/struct.File.html) не реализует `Ord`, поэтому компиляции завершится с ошибкой:
...
Всё работает. Однако, как вы могли заметить, нам приходится явно запаковывать объекты `File` в обёртку `FileWrapper`. К тому же, для доступа к объекту файла внтури обёртки, нам приходится использовать индекс `.0` (в примере: `file.0.metadata()`), что не очень элегантно.

Для решения этой проблем, при использовании newtype паттерна, для типа обёртки принято реализовывать трэйты `From` и `Deref`. Это позволит:
