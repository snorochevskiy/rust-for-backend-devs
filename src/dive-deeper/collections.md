...
* [BinaryHeap\<T>](https://doc.rust-lang.org/std/collections/struct.BinaryHeap.html) — Очередь с приоритетом, основанная на [двоичной куче](https://ru.wikipedia.org/wiki/%D0%94%D0%B2%D0%BE%D0%B8%D1%87%D0%BD%D0%B0%D1%8F_%D0%BA%D1%83%D1%87%D0%B0).

## Vec

Мы уже познакомились с вектором и его устройством в памяти в главе [Вектор](../rust-basics/vector.md).

Давайте просто взглянем на примеры частоиспользуемых методов.
...
В отличии от вектора, двусвязанный список позволяет эффективно вставлять/удалять элементы в середину и начало списка.
...
Двусвязанный список позволяет вставлять и удалять элементы в середине и в начале списка, но его недостатком является то, что операция доступа к элементу по индексу имеет сложность O(n/2), в отличии от сложности O(1) у вектора.
...
Элементы храняться в непрерывном буфере, поделённом на секции — бакеты (bucket).

При вставке новой пары:

1. для ключа вычисляется хеш-код
2. к хеш-коду применяется маска (bucket\_mask), чтобы определить номер бакета, в котором будет хранится эта пара
...
4. Если после вставки, количество элементов (хранится в Control Bytes) в бакете превысило предельное значение (growth\_left), то выделяется новый буфер большего размера, и в него переносятся значения из текущего буфера.
...
С точки зрения API, `BTreeMap` практически идиентичен `HashMap`: в примере для `HashMap` вы можете заменить тип словаря на с `HashMap` на `BTreeMap`, и программа скомпилируется.
