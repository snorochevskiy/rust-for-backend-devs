...
Первое, что бросается в глаза: мы открываем файл для записи, но не закрываем его. Дело том, что для типа `File` реализован трэйт `Drop`: при выходе объекта из скоупа, деструктор сбросит буфер вывода (вызовом `flush`), а после закроет файл.
