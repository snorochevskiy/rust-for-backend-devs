...
* [PartialOrd](https://app.gitbook.com/u/dkQPoX1YHFcD5pDtm1LhqZnDeAW2), [Ord](https://doc.rust-lang.org/std/cmp/trait.Ord.html) — объявляют методы для определения того, какой из двух объектов больше другого (испольуется при сортировке)
...
Многие функции стандартной библиотеки работают с трэйтом `Default`. Например, тип `Option` имеет метод `unwrap_of_default()`, который возвращает значение по умолчанию, в случае вызова на объекте `None`.
