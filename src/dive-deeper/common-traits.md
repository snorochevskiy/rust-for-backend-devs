...
* [Debug](https://doc.rust-lang.org/std/fmt/trait.Debug.html) — определяет функциональность, для получения отладочного текстового описания объекта. Используется при выводе макросом `println!` по средствам форматирующей комбинации `{:?}`
...
* [Hash](https://doc.rust-lang.org/std/hash/trait.Hash.html) — определяет метод для вычисления хэш-кода из объекта
...
* [PartialOrd](https://app.gitbook.com/u/dkQPoX1YHFcD5pDtm1LhqZnDeAW2), [Ord](https://doc.rust-lang.org/std/cmp/trait.Ord.html) — объявляет методы для определения того, какой из двух объектов больше другого (испольуется при сортировке)
...
* [Send](https://doc.rust-lang.org/std/marker/trait.Send.html) — маркерный трэйт, который автоматически добавляется компилятором для типов, чьи значения могут передаваться в другой поток. О нём мы так же поговорим в главе про [Многопоточность](multithreading.md).
...
Для примера, создадим тип "Груз" с двумя полями: масса и флаг — является ли груз хрупким. Мы хотим иметь возможность сортировать вектор с грузами так, чтобы груз с меньшей массой оказывался перед грузом с большей массов, но все хрупкие грузы были перед всеми нехрупкими.
...
Многие функции стандартной бибилиотки работают с трэйтом `Default`. Например, тип `Option` имеет метод `unwrap_of_default()`, который возвращает значение по умолчанию, в случае вызова на объекте `None`.
...
Единственная разница заключается в том, что вся функциональность определённая в стандартной библиотеке для трэйта `Borrow`, предполагает связь в реализаций ряда трэйтов и для владеющего объекта, и для полученной из него ссылки.

А именно, для неких объктов `x` и `y` типа `T`, реализующего трэйт `Borrow<R>`, предполагается, что:
...
Примером не `Sized` типа является тип `str`. Не `&str`, а именно `str`. Да, на самом деле тип строкового литерала — это `str`, но мы всегда работаем с ними по средствам слайс-ссылки `&str`. А слайс-ссылка, как мы знаем, имеет известный и постоянный размер (два поля: указатель и длина). Таким образом `str` — не `Sized`, а `&str` — `Sized`.

По аналогии с `str`, не `Sized` типом является и `[T]` — последовательность элементов в памяти неопределённого размера. Точто так же как и с `str`, с `[T]` мы работаем по средствам слайса `&[T]`, который — `Sized`.
...
Просматривая генерик код в сторонних бибилотеках или стандартной библиотеке, можно часто встретить такое послебление трэйт границы через `?Sized`.
