...
Панику так же можно инициировать вручную при помощи макроса [panic](https://doc.rust-lang.org/std/macro.panic.html). Синтаксис его использования такой же как у макроса `println!`, но вместо вывода на консоль, он завершит программу и напечатает переданное сообщение.
...
Если замыкание отрабатывает без ошибок, то его результат возвращается завёрнутым в `Ok`, если же при выполнении возникла паника, то будет возращено `Err`.
...
Функция `my_func` возвращает значение типа `String`, поэтому вызов макроса `todo` как бы возвращает тип `String`. Это очень удобно, так как можно вставлять `todo` в любом месте, какой бы сложный тип там не ожидался, и компилятор будет удовлетворён.
