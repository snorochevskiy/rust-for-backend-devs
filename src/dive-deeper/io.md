...
В этом модуле находят два базовых трэйта, задающих единый интерфейс для чтения и записи: [Read](https://doc.rust-lang.org/std/io/trait.Read.html) и [Write](https://doc.rust-lang.org/std/io/trait.Write.html).
...
Давайте рассмотрим работу с трэйтом `Read` на примере коллекции `VeqDeque<u8>`, котоую мы изучили в прошлой главе, и которая так же реализует трэйт `Read`.
...
fn main() {
    // Создаём VecDeque, из которого будем читать по средствам трэйта Read
    let mut v: VecDeque<u8> = VecDeque::from(vec![1, 2, 3, 4, 5, 6, 7, 8, 9]);

    // Буфер размером в 2 байта. В него мы будет производить чтение
...
Как видим, `Read` реализован для любой вариации `Cursor`, которая хрант в себе любой тип, реализующий `AsRef<u8>`. Тип `Vec<u8>` как раз реализует `AsRef<u8>`.
...
Метод `flush` имеет смысл для типов, которые перед фактической записью данных, предварельно буферизируют их в памяти. Вызов метода должен приводить к сбросу буферов в конечное хранилище данных.

Так же, трэйт содержит метод с реализацией по умолчанию — `write_all`.

```rust
fn write_all(&mut self, buf: &[u8]) -> Result<()>;
```

Этот метод, в отличии от `write`, должен записывать все байты, даже если это займёт существенно больше времени.

Как и `Read`, трэйт `Write` реализован для файла, сетевого сокета, Unix канала, STDOUT, т.д.

***

В отличии от `Read`, трэйт `Write` реализован не только для `VecDequeue<u8>`, но для `Vec<u8>`, так как вектор позволяет эффективно добавлять элементы в свой конец.

Рассмотрим пример записи байт в `Vec<u8>` по средствам трэйта `Write`:
...
Стандартный вывод STDOUT так же позволяет работать с собой через трэйт `Write`:
