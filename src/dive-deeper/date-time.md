...
Сам по себе тип `Duration` умеет не много, однако он используется как входной аргумент в различной функциональности. Например, стандартная функция [sleep](https://doc.rust-lang.org/std/thread/fn.sleep.html), которая останавливает выполнение потока на указанное время, принимает аргумент типа `Duration`.
...
> [!NOTE]
> Обратите внимание, что типы данных из библиотеки chrono поддерживают сериализацию по средствам [serde](https://crates.io/crates/serde), однако эту функциональность надо отдельно включить при помощи фичи "serde".
...
* добавлять и вычетать годы, месяцы и дни
...
* [Utc](https://docs.rs/chrono/latest/chrono/struct.Utc.html) — UTC часовой пояс (нулевой миридиан)
...
Объект `DateTime` так же может быть создан:
...
Последнее, с чем нам осталось разобраться — `DateTime<FixedOffset>`. Тип `FixedOffset` позволяет задавать любую тайм зону путём указания смещения в секундах относительно UTC. Для примера дайвайте создадим тайм зону UTC+2:30, т.е. смещение на восток от Гринвича на 2 часа 30 минут.
