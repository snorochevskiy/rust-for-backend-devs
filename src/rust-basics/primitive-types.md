...
Тип числа можно указывать, как явно, так и про помощи суфикса к инициализирующему значению:
...
Принципиальное отличие типа Unit, являющегося множеством синглтон, от типа void, который представляет пустое множество, заключается в том, что у типа Unit есть одно единственное значение. Мы можем даже создать переменную типа Unit и присвоить ей единственное возможное значение — `()` .
...
> 
> В чисто функциональном языке Haskell даже имеется специальная фукция `absurd`, которая принимает аргумент типа Void и возвращает некое значение.
> 
> ```haskell
> absurd :: Void -> a
> ```
