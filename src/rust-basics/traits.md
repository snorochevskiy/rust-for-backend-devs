...
Трэйты служат для определения интерфейса взаимодействия с типом путём описания методов, которые должны у него присутствовать. Если для некого типа необходимо реализовать трэйт, то для этого типа нужно определить реализации <ins>всех</ins> методов, объявленных в трэйте.
...
Дело в том, что когда компилятор встречает использование функции, имеющий аргумент типа `impl Трэйт`, то он генерирует вариант этой функции для конкретного типа, с которым функция вызвана.
...
> [!NOTE]
> Сейчас эта секция будет малопонятной. Её лучше перечитать после прохождения главы про [Генерики](generics.md).
