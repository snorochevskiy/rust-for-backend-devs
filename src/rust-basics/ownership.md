...
Тот факт, что у любого объекта есть только один владелец, позволяет компилятору однозначно понять в каком месте память, занимаемая объектом, должны быть очищена — там, где владелец объекта прекращает своё существование.
...
Этот пример тривиален, однако по нему хорошо видно как работает передачать владения при переходе между скоупами. Эти знания нам понадобится в дальнейшем, когда мы будем разбирать [Лайфтаймы](lifetimes.md)
...
fn greet(name: String) -> String {
    format!("Hello {}!!!", name) // строка результат перемещается в вызывающий код
}
```
...
И вот здесь нас сразу ожидает проблема: мы пытаемся распечатать переменную `s`, которая уже не действительная, так как она отдала владение своими данными в вызов функции на предыдущей строке.
...
> [!NOTE]
> В стандартной библоитеке Rust есть функция [drop](https://doc.rust-lang.org/std/mem/fn.drop.html), которая унитожает переданный ей объект.
> 
> ```rust
> let s = String::from("aaa");
> drop(s);
> ```
> 
> Если посмотреть реализацию функции `drop` (мы пока не знакомы с [генериками](generics.md), поэтому не можем полность понять её сигнатуру), то мы увидим, что она не делает ничего.
> 
> ```rust
> pub fn drop<T>(_x: T) {}
> ```
...
Примитивные типы занимают мало памяти, а так же не владеют какими-то дополнительными ресурсами, поэтому в тех ситуациях где для составных типов происходит передача владения, для примитивных типов просто выполняется копирование.
...
> [!NOTE]
> После этого примера может показаться, что написание програм на Rust превращается в постоянную борьбу с компилятором. На первых порах так оно может и быть. Однако, после того как вы привыкнете в концепции владения данными, вы будете на автомате писать так как надо.
