...
В Rust, в отличии от C и Java, тело условной ветки обязательно должно быть обрамлено в фигурные скобки, даже если ветка содержит только одно выражение.

## Результат оператора if

Оператор `if` является выражением, т.е. возвращает значение, которое можно присвоить переменной. Результатом всего `if` выражения является результат последнего выражение в отработавшей ветки.
