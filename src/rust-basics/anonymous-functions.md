...
Анонимные функции компилируются таким же образом как и обычные, и помещаются в сегмент кода. Когда мы присваиваем анонимную функцию переменной, то в переменую просто записывается указатель на функцию, находящуюся в сегменте кода.
...
Как мы сказали выше, замыкание может захватывать значения из внешнего констекста по значению или по ссылке. От того каким образом захваченное значение будет использовано в теле замыкания, будет зависеть то, на основе какого трэйта компилятор сгенерирует тип замыкания: `Fn`, `FnMut` или `FnOnce`.
...
Для примера, напишем еще одно инкрементирующее замыкание. Однако теперь это замыкание будет иметь ссылку на число, которое хранит шаг инкремента. Причём после каждого вызова этот шаг инкремента будет увеличивать на единицу.
