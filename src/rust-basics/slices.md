...
Аналогично тому, как ссылка создаётся оператором `&`, слайc создаётся оператором `&[]`.
...
В отличии от обычных ссылок, которые, как правило, эфемерны (не представлены отдельными ячейками в памяти), слайсы хранятся в памяти в виде пары полей:
