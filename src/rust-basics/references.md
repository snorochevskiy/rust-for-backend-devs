...
Ссылки, как и переменные, по умолчанию являются немутабельным, т.е. с их помощью можно читать значение оригинальной переменной, но не изменять его. Чтобы создать мутабельную ссылку, нужно вместо `&` использовать `&mut`. Разумеется, мутабельную ссылку можно получить только для мутабельной переменной:
...
Мы поговорим подробнее о ссылка в главе [Владение](ownership.md), а так же в главе [Лайфтаймы](lifetimes.md).
