...
Статические переменные создаются в самом начале работы работы программы и существуют до её завершения, однако в отличие от констант, статические переменные:
...
> [!NOTE]
> В то время как константы, как правило, хранятся в сегмента кода, статические переменные хранятся в серменте bss (Block Started by Symbol) или в сегменте данных.
...
Также мы увидим применение таких переменных в главах [Деструктурирующее присваивание](destructuring-assignment.md) и [Паттерн матчинг](pattern-matching.md).
