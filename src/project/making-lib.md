...
Мы уже знаем, что для создание Rust проекта, который компилируется в исполняемый бинарный файл, используется команда `cargo new`. Для создания библиотеки используется та же команда, но с флагом `--lib`:
