...
Обратите внимение, что структура `AccountServiceImpl` инкапсулирует хранилище по средствам `Arc<dyn Storage + Send + Sync>`. Это позволит нам иметь возможность заинжектить в `AccountServiceImpl` как реальное хранилище — `StorageImpl`, так и какую-то заглушка для тестов.  Ограничения `Send` и `Sync` необходимы так методы трэйта асинхронны.

Теперь файл, в котором создаётся Axum сервер — `server/src/lib.rs`. Обратите внимание, что объект нашей бизнес логики — `AccountServiceImpl` мы храним в объекте сосотояния, причём так же не напрямую, а по средствам `Arc<dyn AccountService + Send + Sync>`.
...
Так же напишем интеграционный тест для эндпоинтов `server/tests/endpoints_tests.rs`. Он будет, по средствам вызова эндпоинтов, сначала создавать новый аккаунт, а затем проверять его наличие в хранилище.
...
и вызвать эндпоинты, чтобы убедиться в том, что они работают (не забудте убедиться, что сервер СУБД запущен).
...
Вот и всё: мы разобрались каким образом на Axum и SQLx можно писать бекенды с классической слоёной архитекутрой, где присутствуют слой данных, слой бизнес логики и слой представления.
