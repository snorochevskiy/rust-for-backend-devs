...
Обратите внимение, что структура `AccountServiceImpl` инкапсулирует хранилище посредством `Arc<dyn Storage + Send + Sync>`. Это позволит нам иметь возможность заинжектить в `AccountServiceImpl` как реальное хранилище — `StorageImpl`, так и какую-то заглушка для тестов.  Ограничения `Send` и `Sync` необходимы так методы трэйта асинхронны.

Теперь файл, в котором создаётся Axum сервер — `server/src/lib.rs`. Обратите внимание, что объект нашей бизнес логики — `AccountServiceImpl` мы храним в объекте сосотояния, причём также не напрямую, а посредством `Arc<dyn AccountService + Send + Sync>`.
