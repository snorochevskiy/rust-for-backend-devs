...
Сначала, давайте создайте новую БД с именем `mydb`. Если вы предпочитаете использовать докер, то можете использовать следующие команды:
...
* таймаут для получения запроса из пула, максимальное время жизни соединения, максимальное время жизни простаивающего соединения
...
Результат запроса представлен объектами типа [PgRow](https://docs.rs/sqlx/latest/sqlx/postgres/struct.PgRow.html) (был бы [MySqlRow](https://docs.rs/sqlx-mysql/latest/sqlx_mysql/struct.MySqlRow.html) для MySQL, и [SqliteRow](https://docs.rs/sqlx-sqlite/latest/sqlx_sqlite/struct.SqliteRow.html) для SQLite).
...
В качестве примера рассмотрим функцию, которая совершает перессылку денег с одного аккаунта на другой. Эта функция должна транзакционно совершить три операции:

1. снять деньги со счёта отправителья
...
async fn make_transfer(transfer: &Transfer, pool: &PgPool) -> Result<(),sqlx::Error> {
    ...
    // Создаём новую запись о транзакции по перессылке денег
    let _ = sqlx::query(r#"
...
Как видите, мы попытались перевести 5000, что привело к тому, что вторая UPDATE операции (та которая вычетает сумму со счета отправителя) в функции `make_transfer` завершилась с ошибкой.
