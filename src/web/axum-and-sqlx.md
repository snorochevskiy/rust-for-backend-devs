...
Теперь когда мы разобрались как работать с SQLx, давайте посмотрим как использовать его cовместно с Axum.
...
В результате, у нас должно получится такое дерево файлов:
...
Теперь код нашего сервера. Для того, чтобы эндпоинты могли использовать базу данных, мы просто поместим объекта пула соединений с БД в объект состояния.
...
Если вы использовали docker, чтобы запускать сервер Postgres, то можете вопспользоваться командами:
